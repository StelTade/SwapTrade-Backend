...
export class ComplexityGuard implements CanActivate {
  ...
  private calculateComplexity(obj: any, depth: number): number {
    if (depth > this.MAX_DEPTH) {
      throw new HttpException(
        `Request depth exceeds limit of ${this.MAX_DEPTH}`,
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (Array.isArray(obj)) {
      if (obj.length > this.MAX_ARRAY_LENGTH) {
        throw new HttpException(
          `Array length exceeds limit of ${this.MAX_ARRAY_LENGTH}`,
          HttpStatus.BAD_REQUEST,
        );
      }
      for (const item of obj) {
        score +=
          typeof item === 'object' && item !== null
            ? this.calculateComplexity(item, depth + 1)
            : 1;
      }
      ...
      if (keys.length > this.MAX_KEYS)
        throw new HttpException(
          `Object keys count exceeds limit of ${this.MAX_KEYS}`,
          HttpStatus.BAD_REQUEST,
        );
