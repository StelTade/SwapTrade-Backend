...
export class CacheController {
  ...
  getWarmingStatus(): {
    isWarming: boolean;
    warmingMetrics: CacheWarmingMetrics;
  } {
    return {
  ...
  @Get('redis/metrics')
  @ApiOperation({
    summary: 'Get Redis connection pool and retry metrics (Prometheus-style)',
  })
  @ApiResponse({
    status: 200,
    description: 'Redis pool utilization and retry rates',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Pool size, available, in use, retries',
  })
  getRedisPoolMetrics():
    | ReturnType<RedisPoolService['getMetrics']>
    | { enabled: false } {
