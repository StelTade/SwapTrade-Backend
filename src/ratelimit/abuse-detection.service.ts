...
export class AbuseDetectionService {
  ...
  async trackRequest(
    ...
    if (totalScore >= this.ABUSE_SCORE_THRESHOLD && !isBlocked) {
      ...
      this.logger.warn(
        `Auto-blocked identifier ${identifier} with abuse score ${totalScore}`,
      );
    ...
    return {
      ...
      patterns,
      isBlocked,
      blockExpiresAt: isBlocked
        ? await this.getBlockExpiry(identifier)
        : undefined,
    };
  }
  ...
  private async detectPatterns(identifier: string): Promise<AbusePattern[]> {
    ...
    const requestsPerMinute =
      (requests.length / this.PATTERN_WINDOW_MS) * 60000;
    if (requestsPerMinute > this.RAPID_REQUEST_THRESHOLD) {
      patterns.push({
        ...
        score: Math.min(
          40,
          (requestsPerMinute / this.RAPID_REQUEST_THRESHOLD) * 20,
        ),
        detectedAt: new Date(),
    ...
    const failedAuths = requests.filter(
    ...
    if (
      endpointDiversity > this.ENDPOINT_DIVERSITY_THRESHOLD &&
      requests.length > 20
    ) {
      patterns.push({
        identifier,
    ...
    const suspiciousUAs = requests.filter(
      (r) =>
    ...
    if (suspiciousUAs > requests.length * 0.5) {
      patterns.push({
        ...
        detectedAt: new Date(),
        metadata: {
          suspiciousRequests: suspiciousUAs,
          totalRequests: requests.length,
        },
      });
    }
  ...
  async blockIdentifier(identifier: string, durationMs: number): Promise<void> {
    ...
    this.logger.warn(
      `Blocked identifier: ${identifier} until ${new Date(expiresAt).toISOString()}`,
    );
  }
   ...
   */
  async getBlockedIdentifiers(): Promise<
    Array<{ identifier: string; expiresAt: Date }>
  > {
