...
export function rateLimitMiddlewareFactory(
  rl: RateLimitService,
  ...
  return async function rateLimitMiddleware(
    ...
    try {
      ...
      const { allowed, remaining, reset } = await rl.check(
        identifier,
        endpoint,
      );
      res.setHeader(
        'X-RateLimit-Limit',
        String(config.rateLimit?.maxRequests ?? 100),
      );
      ...
      if (!allowed) {
        res
          .status(429)
          .json({ message: config.rateLimit?.message || 'Too many requests' });
        return;
      }
    } catch (err) {
