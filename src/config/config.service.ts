...
export class ConfigService implements OnModuleInit {
  ...
  private async loadConfiguration(): Promise<void> {
    const configObject = {
      app: {
        nodeEnv: this.nestConfigService.get<Environment>(
          'NODE_ENV',
          Environment.DEVELOPMENT,
        ),
        port: this.nestConfigService.get<number>('PORT', 3000),
        host: this.nestConfigService.get<string>('HOST', 'localhost'),
        enableShutdownHooks: this.nestConfigService.get<boolean>(
          'ENABLE_SHUTDOWN_HOOKS',
          true,
        ),
        shutdownTimeout: this.nestConfigService.get<number>(
          'SHUTDOWN_TIMEOUT',
          30000,
        ),
        cors: {
          ...
          methods: this.nestConfigService
            .get<string>('CORS_METHODS', 'GET,POST,PUT,DELETE,PATCH')
            .split(','),
          allowedHeaders: this.nestConfigService
            .get<string>('CORS_ALLOWED_HEADERS', 'Content-Type,Authorization')
            .split(','),
        },
        swagger: {
        ...
        logging: {
          ...
          enableConsole: this.nestConfigService.get<boolean>(
            'LOG_ENABLE_CONSOLE',
            true,
          ),
          enableFile: this.nestConfigService.get<boolean>(
            'LOG_ENABLE_FILE',
            true,
          ),
      ...
      database: {
        ...
        autoLoadEntities: this.nestConfigService.get<boolean>(
      ...
      redis: {
        ...
        retryDelayOnFailover: this.nestConfigService.get<number>(
          'REDIS_RETRY_DELAY_ON_FAILOVER',
          100,
        ),
        enableReadyCheck: this.nestConfigService.get<boolean>(
          'REDIS_ENABLE_READY_CHECK',
          false,
        ),
        maxRetriesPerRequest: this.nestConfigService.get<number>(
        ...
        backoffBaseMs: this.nestConfigService.get<number>(
          'REDIS_BACKOFF_BASE_MS',
          100,
        ),
        backoffMaxMs: this.nestConfigService.get<number>(
          'REDIS_BACKOFF_MAX_MS',
          30000,
        ),
        backoffMaxAttempts: this.nestConfigService.get<number>(
          'REDIS_BACKOFF_MAX_ATTEMPTS',
          10,
        ),
        circuitBreakerThreshold: this.nestConfigService.get<number>(
          'REDIS_CIRCUIT_BREAKER_THRESHOLD',
          5,
        ),
        circuitBreakerResetMs: this.nestConfigService.get<number>(
      ...
      cache: {
        ...
        ttlConfig: {
          userBalances: this.nestConfigService.get<number>(
            'CACHE_TTL_USER_BALANCES',
            30,
          ),
          marketPrices: this.nestConfigService.get<number>(
            'CACHE_TTL_MARKET_PRICES',
            300,
          ),
          portfolio: this.nestConfigService.get<number>(
            'CACHE_TTL_PORTFOLIO',
            60,
          ),
          tradingData: this.nestConfigService.get<number>(
            'CACHE_TTL_TRADING_DATA',
            10,
          ),
          userProfile: this.nestConfigService.get<number>(
        ...
        warming: {
          ...
          timeout: this.nestConfigService.get<number>(
            'CACHE_WARMING_TIMEOUT',
            30000,
          ),
          strategies: this.nestConfigService
            .get<string>(
      ...
      queue: {
        ...
        maxStalledCount: this.nestConfigService.get<number>(
          'QUEUE_MAX_STALLED_COUNT',
          10,
        ),
        stalledInterval: this.nestConfigService.get<number>(
          'QUEUE_STALLED_INTERVAL',
          30000,
        ),
        removeOnComplete: this.nestConfigService.get<boolean>(
          'QUEUE_REMOVE_ON_COMPLETE',
          true,
        ),
        removeOnFail: this.nestConfigService.get<boolean>(
      ...
      auth: {
        ...
        jwtExpiresIn: this.nestConfigService.get<number>(
          'JWT_EXPIRES_IN',
          3600,
        ),
        jwtRefreshSecret:
          this.nestConfigService.get<string>('JWT_REFRESH_SECRET'),
        jwtRefreshExpiresIn: this.nestConfigService.get<number>(
          'JWT_REFRESH_EXPIRES_IN',
          604800,
        ),
        bcryptRounds: this.nestConfigService.get<number>('BCRYPT_ROUNDS', 12),
        maxLoginAttempts: this.nestConfigService.get<number>(
          'AUTH_MAX_LOGIN_ATTEMPTS',
          5,
        ),
        lockoutDuration: this.nestConfigService.get<number>(
          'AUTH_LOCKOUT_DURATION',
          900000,
        ),
      },
      rateLimit: {
        windowMs: this.nestConfigService.get<number>(
          'RATE_LIMIT_WINDOW_MS',
          900000,
        ),
        maxRequests: this.nestConfigService.get<number>(
          'RATE_LIMIT_MAX_REQUESTS',
          100,
        ),
        skipSuccessfulRequests: this.nestConfigService.get<boolean>(
          'RATE_LIMIT_SKIP_SUCCESSFUL_REQUESTS',
          false,
        ),
        skipFailedRequests: this.nestConfigService.get<boolean>(
          'RATE_LIMIT_SKIP_FAILED_REQUESTS',
          false,
        ),
        keyGenerator: this.nestConfigService.get<string>(
          'RATE_LIMIT_KEY_GENERATOR',
          'req.ip',
        ),
        standardHeaders: this.nestConfigService.get<boolean>(
          'RATE_LIMIT_STANDARD_HEADERS',
          true,
        ),
        message: this.nestConfigService.get<string>(
          'RATE_LIMIT_MESSAGE',
          'Too many requests from this IP, please try again later.',
        ),
      },
      features: {
        enableAdvancedCaching: this.nestConfigService.get<boolean>(
          'FEATURE_ADVANCED_CACHING',
          true,
        ),
        enableQueueMonitoring: this.nestConfigService.get<boolean>(
          'FEATURE_QUEUE_MONITORING',
          true,
        ),
        enableRateLimiting: this.nestConfigService.get<boolean>(
          'FEATURE_RATE_LIMITING',
          true,
        ),
        enableAuditLogging: this.nestConfigService.get<boolean>(
          'FEATURE_AUDIT_LOGGING',
          true,
        ),
        enableHotReload: this.nestConfigService.get<boolean>(
          'FEATURE_HOT_RELOAD',
          false,
        ),
        enableABTesting: this.nestConfigService.get<boolean>(
          'FEATURE_AB_TESTING',
          false,
        ),
        enablePerformanceMonitoring: this.nestConfigService.get<boolean>(
          'FEATURE_PERFORMANCE_MONITORING',
          true,
        ),
        enableErrorTracking: this.nestConfigService.get<boolean>(
  ...
  private setupHotReload(): void {
    ...
    this.envFilePath = this.nestConfigService.get<string>(
      'ENV_FILE_PATH',
      '.env',
    );
    ...
    if (fs.existsSync(this.envFilePath)) {
      this.watcher = fs.watch(this.envFilePath, (eventType) => {
        if (eventType === 'change') {
          this.logger.log(
            'Environment file changed, reloading configuration...',
          );
