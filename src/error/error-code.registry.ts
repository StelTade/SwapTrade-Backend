...
export class ErrorCodeRegistry {
   ...
   * Get all error codes
   */
  static getAllErrorCodes(): Record<
    ErrorCode,
    (typeof ERROR_CODES)[ErrorCode]
  > {
    return ERROR_CODES;
  }
   ...
   */
  static getErrorCodesByCategory(
    category: string,
  ): Record<string, (typeof ERROR_CODES)[ErrorCode]> {
   ...
   */
  static getSortedErrorCodes(): Array<{
    code: ErrorCode;
    definition: (typeof ERROR_CODES)[ErrorCode];
  }> {
    return Object.entries(ERROR_CODES)
      .map(([code, definition]) => ({ code: code, definition }))
      .sort(
        (
          a: { code: ErrorCode; definition: (typeof ERROR_CODES)[ErrorCode] },
